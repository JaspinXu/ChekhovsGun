...
ChekhovsGun 是一个本地优先的浏览器扩展：保存网页与视频，在你阅读相关内容时，
从自己的收藏中找出有用的段落，并提供原文或视频时间点链接。

在支持的站点照常点击收藏，或使用扩展工具栏保存当前页面。
你也可以随时搜索已保存的内容，不必等提醒出现。采集、索引和检索默认在浏览器内完成；
可选 Python 后端补充平台同步、字幕与评论采集和本地语音转写。
...
- **顺手保存：** 在支持的站点点击收藏按钮即可采集，也可通过扩展工具栏保存当前页面。
- **随时搜索：** 从第一条收藏起即可主动搜索，查看相关段落并跳转原文。
- **刷到就提醒：** 扩展识别你当前在看的页面，命中就弹一张卡片——视频跳到讲这件事的那一秒，帖子滚动到并高亮出讲这件事的那一段。
- **收藏夹扫描：** 收集列表滚动加载出的链接；独立模式先存标题和链接，扫描范围受站点分页和虚拟列表限制。
...
- **多种内容源：** 扩展抽取网页正文、视频标题和简介；可选后端补充字幕、高赞评论与本地 Whisper 转写。

...
想着"以后一定看"，然后它就永远躺在收藏夹里吃灰了。藏知把这件事反过来做——
...
```
  两条进货路                    索引                      刷到东西时
┌───────────────┐            ┌──────────────┐         ┌──────────────┐
│ API 适配器     │   字幕     │  分块 + 向量  │         │  浏览器扩展   │
│ YouTube · B站 │───────────▶│  + BM25 倒排 │◀────────│  读你当前     │
├───────────────┤            │   SQLite     │ 混合检索 │  在看的页面   │
│ 浏览器采集     │   正文     │              │         └──────┬───────┘
│ 知乎·小红书·…  │───────────▶│              │                │
└───────────────┘            └──────────────┘                │
...
1. **收进来**，两条路最后汇到同一个地方：
   - **适配器**从 YouTube 播放列表 / Liked / Watch Later 和 B 站收藏夹拉取你保存过的
     视频，连同字幕（B 站 CC 字幕与 AI 字幕都要）和**高赞评论**。完全没有字幕的，
     用本地 Whisper 转写兜底。
   - **浏览器扩展**负责那些没有可用 API 的站点——知乎、小红书、公众号、微博、
     掘金、Reddit……它读的是你**已经登录的浏览器**看得到的页面，所以不需要你交出任何
     凭据。另外还有一键扫描，把你收藏夹里积压的存量整个收进来。
...
先做这一步。它是唯一完全不需要任何凭据的部分，而且覆盖了适配器够不到的所有站点。

1. Chrome / Edge 打开 `chrome://extensions`
2. 打开右上角「开发者模式」
3. 点「加载已解压的扩展程序」，选择本仓库的 `extension/` 目录
4. 确保 `chekhovsgun serve`（或 `chekhovsgun tray`）正在运行

之后你照常上网就行：

- **点网站自己的「收藏」按钮**，这一页就进来了。知乎、小红书、公众号、微博、掘金、
  CSDN、简书、Reddit、X、Stack Overflow、Medium 都内置了规则，YouTube 和 B 站也一样。
- **在收藏夹页面**上会出现一个按钮：*把这个收藏夹收进藏知*。它会自动往下翻完整个收藏夹，
  把每一条链接都收进来，正文在后台慢慢抓——**你积压的存量就是这样一次性收进来的**。
- **其它任何页面**，点扩展图标再点*收进藏知*。这条路走的是 `activeTab`：
  只在你点的那一下授权，扩展平时没有读取你浏览记录的权限。

扩展只访问 `http://127.0.0.1:8700`，不做任何其他网络请求。
端口、冷却时间、每个站点的开关、是否自动采集，都在扩展的选项页里。

> 有些网站收藏和取消收藏是同一个按钮。控件自己暴露了状态的，我们按状态判断；
> 判断不出来的，按「收藏」处理。多收进来的条目可以在仪表盘里删掉。

加一个新站点只要在 `extension/recipes.js` 里加一条记录，不用动 Python——
服务端会自己把 URL 归一化并认出来源。完全没有规则的站点也能用（走工具栏按钮）：
通用抽取器会挑出**正文密度最高、且不在链接里**的那个元素，这在绝大多数页面上就是正文。
